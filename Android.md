...
Verified during on-device bring-up (Samsung SM-G780G) — this supersedes the "likely
r29 / may be required" hedging in §7–§8.

The full reference — the library-vs-app ownership split, the Expo-vs-bare-RN
mechanics, the per-setting table, and the decision to document `extraMavenRepos`
rather than ship a library config plugin — lives in **[`RN.md`](./RN.md)** (single
source of truth). In short:

- **Library owns** its dependency source of truth (Switchboard Maven repo — autolinked
  on bare RN, documented `extraMavenRepos` on Expo) + the C++/Prefab wiring +
  `RECORD_AUDIO`/`MODIFY_AUDIO_SETTINGS` manifest merge.
- **App owns** its build toolchain (NDK r29, drop-x86, `useLegacyPackaging`) — on Expo,
  drop-x86 + packaging via `expo-build-properties` and NDK via a documented
  `app/build.gradle` edit (no config plugin); on bare RN, committed
  `gradle.properties`/`app/build.gradle` edits.
- We **don't** force NDK/ABIs/packaging from a library plugin (beyond ecosystem norms).
